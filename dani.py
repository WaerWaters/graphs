...
def get_graph_data(place_names, display=False, intersection_focus=False, focus_ignore_edges=False, intersection_edges_quanity=3, maxspeed_fallback=25, basic_stats_toggle=True, interactive_map=False, simple=True, centrality=False, save_data=True, save_data_format='gpkg', features=None):
    ...
    for place_name in place_names:
        ...
        if intersection_focus == True:
            ...
            for node in node_degrees:           
                ...
                if save_data:
                    if save_data_format == 'tab':
                        for roads in edges:
                            try:   
                                row = {
                                    'from_node': roads[0],
                                    'to_node': roads[1],
                                    'road_id': roads[2]['osmid'],
                                    'road_length': roads[2]['length'],
                                    'max_speed': roads[2]['speed_kph'],
                                }
                                print('DOne')
                            except:
                                number += 1
        ...
        if save_data and save_data_format == 'tab':
            df.to_csv(f'edges_for_{place_name}', sep=';')
    ...
    #print(save_storage['Oakland, California, USA']['9931357410'])
    #print(df)
...
graph_data = get_graph_data(place_names=places, display=False, intersection_focus=False, focus_ignore_edges=False, intersection_edges_quanity=3, maxspeed_fallback=30, basic_stats_toggle=True, interactive_map=False, simple=True, centrality=False, save_data=True, save_data_format='tab', features={'amenity': ['kindergarten','school', 'bar', 'fast_food', 'pub', 'college', 'library', 'university', 'courthouse', 'police', 'prison', 'hospital'], 'public_transport': 'stop_position'})
#print(graph_data)
